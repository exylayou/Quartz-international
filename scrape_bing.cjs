...
const queries = [
    "Silestone Miami Vena slab",
    "Silestone Pietra slab",
    "Silestone Yukon slab",
    "Silestone Miami White slab",
    "Silestone Halcyon slab"
];
...
async function main() {
    ...
    for (const query of queries) {
        ...
        if (url) {
            ...
            const name = query.replace('Silestone ', '').replace(' slab', '').toLowerCase().replace(/ /g, '-');
            const filename = `src/assets/images/silestone-${name}.jpg`;
